...
func (s *Store) ReadAgentLiveSnapshot(ctx context.Context, agentID string) (AgentLiveSnapshot, error) {
	...
	var planMode int
	if err := tx.QueryRowContext(ctx, `SELECT id, COALESCE(workline_id,''), type, COALESCE(subagent_type,''), title, model, COALESCE(system_prompt,''), permission_mode, COALESCE(entity_generation,1), COALESCE(permission_generation,1), status, plan_mode, COALESCE(cwd,''), message_count, COALESCE(context_summary,''), COALESCE(prune_boundary_message_id,''), COALESCE(pruned_percent,0), created_at, updated_at FROM agents WHERE id = ?`, agentID).Scan(
		...
		&snapshot.Agent.SystemPrompt, &snapshot.Agent.PermissionMode, &snapshot.Agent.EntityGeneration, &snapshot.Agent.PermissionGeneration,
	...
	messageRows, err := tx.QueryContext(ctx, `SELECT id, agent_id, COALESCE(run_id,''), role, COALESCE(content_json,''), COALESCE(content_text,''), COALESCE(parent_tool_use_id,''), COALESCE(command_text,''), COALESCE(created_by,''), created_at FROM agent_messages WHERE agent_id = ? ORDER BY created_at ASC`, agentID)
	if err != nil {
		return AgentLiveSnapshot{}, err
	}
	for messageRows.Next() {
		var message Message
		var raw string
		if err := messageRows.Scan(&message.ID, &message.AgentID, &message.RunID, &message.Role, &raw, &message.ContentText, &message.ParentToolID, &message.CommandText, &message.CreatedBy, &message.CreatedAt); err != nil {
			messageRows.Close()
			return AgentLiveSnapshot{}, err
		}
		if raw != "" {
