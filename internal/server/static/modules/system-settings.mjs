  ...
  function renderUserSettingsContent() {
    ...
    <div class="settings-live-page users-page">
      <section class="settings-hero-card users-hero-card">
        <div>
          <div class="settings-hero-kicker">${escapeHtml(t("systemSettings.users.kicker"))}</div>
          <div class="settings-hero-title">${escapeHtml(hasUsers ? t("systemSettings.users.hasUsersTitle") : t("systemSettings.users.noUsersTitle"))}</div>
          <p>${escapeHtml(t("systemSettings.users.description"))}</p>
        </div>
        <div class="settings-action-row">
          <button id="refreshAuthStatusBtn" class="settings-action-btn primary" type="button">${escapeHtml(t("systemSettings.users.refresh"))}</button>
        </div>
      </section>
      <div class="settings-status-strip">
        <div><strong>${escapeHtml(status ? (hasUsers ? t("systemSettings.users.initialized") : t("systemSettings.users.uninitialized")) : t("systemSettings.users.loading"))}</strong><span>${escapeHtml(t("systemSettings.users.userStatus"))}</span></div>
        <div><strong>${escapeHtml(status ? (registrationOpen ? t("systemSettings.users.open") : t("systemSettings.users.closed")) : t("systemSettings.users.loading"))}</strong><span>${escapeHtml(t("systemSettings.users.registration"))}</span></div>
        <div><strong>${escapeHtml(state.settings?.version || "0.1.0-dev")}</strong><span>${escapeHtml(t("systemSettings.users.instanceVersion"))}</span></div>
      </div>
      ${state.authError ? `<div class="settings-inline-alert">${escapeHtml(state.authError)}</div>` : ""}
      ${status ? renderAuthStatusSummary(status) : `<div class="settings-empty-card">${escapeHtml(t("systemSettings.users.loadingEmpty"))}</div>`}
    </div>
  `;
  }
