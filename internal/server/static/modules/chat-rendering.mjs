...
export function createChatRenderingController({
  ...
  function applyMessageSnapshot(messages, agentId = state.agent?.id) {
    ...
    el.innerHTML = `${normalized.map(renderChatMessageHTML).join("")}${liveToolCards}${runSummaryCard}${approvalCards}`;
      ...
      <div class="message ${presentation.roleClass} chat-message chat-flow-item chat-flow-${presentation.alignment}" data-chat-alignment="${presentation.alignment}" data-message-role="${escapeAttr(presentation.normalizedRole)}">
        <div class="message-head">
          <div class="message-meta">
            <div class="message-role">${escapeHtml(presentation.role)}</div>
            ${timeHTML}
          </div>
          <button class="message-copy-btn" type="button" data-copy-message="${escapeAttr(String(index))}" title="${escapeAttr(cr("message.copyTitle"))}">${escapeHtml(cr("message.copy"))}</button>
        </div>
