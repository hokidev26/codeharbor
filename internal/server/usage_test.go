...
func TestUsageSummaryRouteReturnsDatabaseStats(t *testing.T) {
	...
	_, _, narrator, err := store.CreateProject(ctx, "Demo", "", t.TempDir(), "openai:gpt-4.1-mini", "acceptEdits")
	if err != nil {
		t.Fatal(err)
	}
	userMessage, err := store.AddMessage(ctx, db.Message{NarratorID: narrator.ID, Role: "user", ContentText: "hello"})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := store.AddMessage(ctx, db.Message{NarratorID: narrator.ID, Role: "assistant", ContentText: "hi"}); err != nil {
		t.Fatal(err)
	}
	if _, err := store.AddToolCall(ctx, db.ToolCall{NarratorID: narrator.ID, MessageID: userMessage.ID, ToolUseID: "tool-1", ToolName: "Read", Status: "succeeded", DurationMS: 120}); err != nil {
		t.Fatal(err)
	}
	if _, err := store.AddToolCall(ctx, db.ToolCall{NarratorID: narrator.ID, ToolUseID: "tool-2", ToolName: "Read", Status: "failed", DurationMS: 30, ErrorMessage: "boom"}); err != nil {
		t.Fatal(err)
	}
	...
	if body.Counts.Projects != 1 || body.Counts.Chapters != 1 || body.Counts.Narrators != 1 {
