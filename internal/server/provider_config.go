...
func providerConfigFromUpdateRequest(providerName string, existing config.ProviderConfig, req providerConfigUpdateRequest) (config.ProviderConfig, error) {
	...
	switch providerType {
	case "openai-compatible", "anthropic", "openai", config.ProviderTypeCodex:
	default:
		return config.ProviderConfig{}, errors.New("API 协议当前仅支持 codex、openai-compatible、anthropic 或 openai")
	}
